...
* [x] Chọn 1 chủ đề thực tế từ tệp `docs/DANH_SACH_DE_TAI.md`: Trợ lý Nhân sự VinFast (Gợi ý 2.1).
* [x] Điền bảng Scoring Matrix 4 tiêu chí Agentic Fit vào file `docs/trace_eval.md`.
* [x] Khai báo Tool Schema đúng chuẩn JSON Schema cho `submit_leave_request` vào file `src/tools.py`.
* [x] Thêm 5 câu test case thực tế vào file `config/test_cases.json`.

---

### 🔷 PHẦN 2 (60 phút): ReAct Agent & MCP Server
* [x] Hoàn thiện hàm thực thi gọi Tool theo chuẩn giao thức MCP JSON-RPC 2.0 trong `src/mcp_server.py`.
...
* [x] Kiểm tra tên Repo cá nhân đúng chuẩn: **`K4A-DAY03-LuuXuanDung-2A202602746`**.
* [x] Chạy lệnh Git để push toàn bộ mã nguồn lên GitHub cá nhân:
  ```bash
  git add .
  git commit -m "feat: complete Day 03 Lab VinFast HR ReAct Agent with MCP"
